...
> `matlab-openalex-pipeline >= v0.1.0`
> (validated with cursor-based standard JSONL output)
...
This allows downstream code to rely on the presence and types of error fields
...
As of **v0.2.3**, the v0.2.x line is considered complete for the normalization scope.
No additional CSV schemas are planned within the v0.2 line.
...
>   multiple institutions. Full expansion is provided via `institutions.csv`
>   (optional; v0.2.3+).
...
>   URL fields are intentionally kept out of the normalization core and should
>   be handled downstream if needed.
...
>   Plaintext reconstruction and NLP processing are intentionally out of scope
>   for this repository and should be handled downstream.
...
- **Citation or reference analysis**
  - See: `matlab-openalex-edges` (separate repository)
